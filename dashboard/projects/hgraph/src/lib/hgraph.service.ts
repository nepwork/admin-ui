...
export class HgraphService {
  ...
  get edges(): BinaryEdge[] { return this.edgeArr.length ? this.edgeArr.slice() : []; }
  get hedges(): Edge[] { return this.hedgeArr.length ? this.hedgeArr.slice() : []; }
  get id(): string { return this.idStr; }
  get nodes(): Node[] { return this.nodeArr.length ? this.nodeArr.slice() : []; }
  get graph(): Graph { return new Graph(this.idStr, this.nodeArr, this.edgeArr); }
   ...
   */
  getNodeInGraph(node: string): Node | undefined {
    return (
      this.nodes.find(n => n.id === node)
    );
  }
   ...
   */
  getEdgeInGraph(e: BinaryEdge): BinaryEdge {
    return ({
      id: e.id,
      from: e.from,
      to: e.to
